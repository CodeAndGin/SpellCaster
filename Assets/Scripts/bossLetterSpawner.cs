...
public class bossLetterSpawner : MonoBehaviour {
	...
	void createNewLetter (char first) {
		if (gameObject.name == "BossNameLetterSpawner (0)") {
			spawnLetter(first);
		} else {
			string alphabet = "abcdefghijklmnopqrstuvwxyz";
			char randomLetter = alphabet[Random.Range(0,26)];
			spawnLetter(randomLetter);
		}
	}

	void spawnLetter (char output) {
		string convert = "";
